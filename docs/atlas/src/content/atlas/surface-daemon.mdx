...
| Endpoint supervisor — state machine, driver ops (`spawn`/`waitForPidGone`), composed restart | a **separate** `@kolu/surface-daemon-supervisor` package *(built in `server/src/ptyHost/` at B2; extracted at S1)* | Runs in the *client* process (kolu-server; the odu CLI or odu-web), and is on a **different volatility axis** from the daemon half — see the decision below. Shared types (`DaemonExit`, the gate's file format) cross a one-directional `workspace:*` edge: the supervisor imports them from `@kolu/surface-daemon`. |
...
The decision (settled 2026-06-12; see [the decision](#s1--a-separate-supervisor-package-not-a-supervisor-subpath)): at S1 the supervisor becomes a **separate package** `@kolu/surface-daemon-supervisor`, *not* a `/supervisor` subpath of this one. The package boundary then **is** the hash boundary, with nothing to configure — kaval hashes the daemon package and not the supervisor one, and the closure test's reachable-from-daemon-entry set stays correct by construction (supervisor code is reached only from server, never from `bin.ts`/`index.ts`). The rejected alternative — a `/supervisor` subpath — would force `default.nix`'s fileFilter to carve `src/daemon/**` out of `src/**`, a subdir glob that silently mis-scopes the key when a file lands in the wrong subdir: #1034's first row, simulated by hand where a package boundary gives it for free.

## S1 · a separate supervisor package, not a `/supervisor` subpath

Recorded as a decision so S1 doesn't relitigate it. The two halves — daemon (this package) and supervisor (`endpoint`/`waitForPidGone`/`restart`, server-side until S1) — could ship as one package with two entries, or as two packages. **Two packages**, because:
...
<Roadmap>
  <Milestone status="next" label="S0 · the daemon half is born as the package (kaval B1)">Revised 2026-06-12: kaval B1 creates `@kolu/surface-daemon` itself, holding the daemon half — `acquirePidGate` plus the gate's file-format primitives (`gatePid`/`isHolderLive`) the supervisor composes, and the `daemonMain` skeleton (gate → serve → SIGTERM teardown; lifetime `forever | idleTimeout(ms)`) — with kaval's entry a ~20-line composition over it. Rationale: review isolation (the mechanism is reviewed once, as a package) and one home for the gate's file format. The package is hashed whole into kaval's staleKey (third root), so its standing invariant is *only daemon-running code lives here* — no supervisor reader; the supervisor *composes* the daemon primitives where it lives. B2 then builds the supervisor half (endpoint states · drivers · `waitForPidGone` · composed restart) in `server/src/ptyHost/` — layout-disciplined and parameterized, but **not** in the package. kaval's B1 e2e (the contract corpus over a real daemon's socket + the gate-race choreography with real processes) is the spine's first soak harness; B2's recycle-on-every-deploy then soaks the restart race in production with zero sessions at stake.</Milestone>
  <Milestone status="next" label="S1 · the post-B extraction">Two moves: the handshake fragment into `@kolu/surface`, and the supervisor half out of `server/src/ptyHost/` into a **separate** `@kolu/surface-daemon-supervisor` package (the [decision](#s1--a-separate-supervisor-package-not-a-supervisor-subpath) above — package boundary, not a `/supervisor` subdir glob). No staleKey re-scope to get wrong: the package boundary IS the hash boundary, so kaval keeps hashing only the daemon package. **Exit: kaval's e2e green with zero behavior change, and — by construction, not by a glob — a daemon-package edit flips kaval's staleKey while a supervisor-package edit does not.**</Milestone>
